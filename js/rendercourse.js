...
function filterResources(filter) {
    ...
    section.innerHTML = filtered.map((resource) => {
            return `<div class="card"><iframe src="https://www.youtube.com/embed/${resource.yt}"></iframe><input type="checkbox" /><div class="col"><h1>${resource.name} | ${resource.duration}</h1><p>${resource.desc}</p></div></div>`;
        })
        .join('');
}
